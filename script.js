...
const ids = ["size","cost","import","fit","self","grant","vat","deg","usage","orient","tilt","shade"];
ids.forEach(id => {
...
function update() {
  ...
  const usage = s("usage"), orient = s("orient"), tilt = s("tilt"), shade = s("shade")/100;
  ...
  usageVal.textContent = usage.toFixed(0); orientVal.textContent = orient.toFixed(0);
